...
export abstract class AbstractApiClient implements IApiClient {
  ...
  protected mintRpcId(): RpcId {
    // crypto.randomUUID is unavailable on insecure origins (plain HTTP on a
    // non-localhost host); crypto.getRandomValues is exposed everywhere, so
    // mint the RFC 4122 v4 id from it instead.
